...
```{r list}
folder<-"C:/Users/jhdiaz/Desktop/Lab stuff/GI Data/TUNEL/TUNEL DATA"
...
dat<-dat%>%mutate(
  Group=case_when(
    grepl(pattern="Results",x=path)~"Negative",
    grepl(pattern="Summary",x=path)~"Positive"
  )
)
...
```{r}
dat<-pivot_wider(dat,names_from = "Group",values_from = "area")
```

```{r}

dat<-dat%>%mutate(
  Percent_Area=Positive/Negative*100
)
...
names(dat)<-c("Time","Ion","Mouse","ROI","Treatment","Negative","Positive","Percent_Area")
...
#write.xlsx(dat,"Image Data Categorized.xlsx")
```

```{r}
dat_summary<-dat%>%group_by(Time,Ion,Treatment,Mouse)%>%
  summarise(
    Percent_Area_Mean=mean(Percent_Area),
    Percent_Area_SEM=std.error(Percent_Area)
    )

write.xlsx(dat_summary,"Image Mouse Summary.xlsx")
#browseURL("Image Mouse Summary.xlsx")

time_summary<-dat%>%group_by(Time,Ion,Treatment)%>%
  summarise(
    Mean=mean(Percent_Area),
    SEM=std.error(Percent_Area))

#write.xlsx(time_summary,"Image Time Summary.xlsx")
```
...
```{r}
#No Treatment
grouped_ggbetweenstats(dat%>%filter(Treatment=="No Treatment"),x="Time",y="Percent_Area",outlier.tagging = TRUE,grouping.var = "Ion")
...
```{r Remove Outliers}
dat<-dat%>%group_by(Time,Ion,Treatment)%>%mutate(
  IQR=IQR(Percent_Area),
  lowerQ=quantile(Percent_Area, probs=c(.25, .75), na.rm = FALSE)[1] - 1.5 * IQR,
  upperQ=quantile(Percent_Area, probs=c(.25, .75), na.rm = FALSE)[2] + 1.5 * IQR
)

dat_no_outliers<-dat%>%filter(
  Percent_Area>lowerQ & Percent_Area<upperQ
)
...
ttest_summary_no<-dat_summary_no%>%
  filter(Treatment=="No Treatment",
         Ion!="Sham")%>%
  group_by(Ion)%>%
  t_test(Percent_Area_Mean~Time,p.adjust.method = "holm")%>%
  add_xy_position()
...
```{r Plot - No Treatments - Time by Percent Area}
#prevent scientific notation in graph
options(scipen = 10000,digits=5)

plot<-function(df,t_test_table,fill_color){
  ggline(df,x="Time",y="Percent_Area_Mean",fill = fill_color,color="black",add=c("mean_se","jitter"))+
  stat_compare_means(method="anova",label.y.npc = .4,label="p.format")+
  stat_pvalue_manual(t_test_table,hide.ns = TRUE,bracket.nudge.y = -.05,step.increase = .02)+
  scale_y_continuous(breaks = c(0,.10,.20,.3),limits = c(0,.35),expand=c(0,0))+
  ylab("Apoptotic cells (% Area)")+
...
```{r Plot - No Treatments - Ion by Percent Area for each Time}
#change y position of first sig bar by this amount
ymin=0.1


timeplot<-function(tabley,ttesttable){
  ggbarplot(tabley,x="Ion",y="Percent_Area_Mean",color="black",fill= "Ion",add=c("mean_se","jitter"),error.plot = "upper_errorbar", order = c("Sham","Gamma","Proton","Iron"))+
  stat_compare_means(method="anova",label.y.npc = .9,label="p.format")+
  scale_fill_manual(values =  c("#ababab","#818181","#333333","black")) +
  stat_pvalue_manual(ttesttable,hide.ns = TRUE,step.increase = .02)+
  scale_y_continuous(breaks = c(0,.10,.20,.3),limits = c(0,.35),expand=c(0,0))+
  ylab("Apoptotic cells (% Area)")+
  xlab("")+
  theme(
  ...
  name=paste0("No Outliers - No Treatment - Ion by Percent Area - ",temp$Time[1],".svg")
...
temp<-dat_summary_no%>%
  ...
  summarise(
    SEM=std.error(Percent_Area_Mean),
    Mean=mean(Percent_Area_Mean),
    Count=n()
  )%>%
  mutate(
...
ttest<-temp1%>%
group_by(Time,Ion)%>%
t_test(Percent_Area_Mean~Treatment,p.adjust.method = "holm")%>%
...
ggplot(temp,aes(Time,Mean,fill = Ion,pattern=Treatment))+
  ...
  scale_y_continuous(expand = c(0,0),limits = c(0,.35),breaks = c(0,.1,.2,.3))+
  ...
  geom_signif(annotations = "*",y_position = .2,xmin=3.275,xmax=3.405,tip_length = .01)+
...
ggsave("TUNEL(cnlp).svg",width=7,height=5)
browseURL("TUNEL(cnlp).svg")
